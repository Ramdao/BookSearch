...
function filter_search() {
    ...
    if (!filterCriteria || !searchValue) {
        document.getElementById("result").innerHTML = "Please enter a filter and enter a search term.";
        return;
    }
        ...
        if (request.status === 200) {
            ...
            if (response.length > 0) {
                ...
                for (let i =0; response.length>i;i++){
                    ...
                    resultHTML += "Author: " + response[i].author + ". Title: " + response[i].title +"Genre: "+response[i].genre+"Publication date: "+response[i].publication_date+"ISBN: "+response[i].ISBN +"<br>";
...
function addbook(){
    ...
    const valid2 = yearPattern.test(document.getElementById("publication_date").value);
    if(!valid){
